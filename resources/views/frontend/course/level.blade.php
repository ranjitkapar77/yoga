            ...
            <div class="row align-items-end">
                <div class="col-lg-12">
                    <div class="page-title-wrapper mb-50">
                       <h1 class="page-title mb-25">{{$level->title}}</h1>
                       <div class="breadcrumb-list">
                          <ul class="breadcrumb">
                              <li><a href="@php echo url('/'); @endphp">Home - Pages -</a></li>
                              <li><a href="{{url()->current()}}">{{$level->title}}</a></li>
                          </ul>
                       </div>
                  </div>
                </div>
            ...
            <div class="row">
                <div class="col-xl-12">
                     <div class="section-title text-center mb-30">
                         <h5 class="bottom-line mb-25">Study {{$level->title}}</h5>
                         @if(count($courses) >= '1')
                            <h2>Explore our Popular Courses</h2>
                         @else
                            <h2>No Any Courses for {{$level->title}}</h2>
                         @endif
                     </div>
                 </div>
             </div>
